...
<head>
	...
	while($result_sp = mysqli_fetch_array($select_sp)){
		$chart_name .= '"'.$result_sp['name'].'", ';

		$select_sale = mysqli_query($con, "SELECT SUM(i.total) FROM invoice i, invoice_details d WHERE i.id = d.invoice_id AND d.prod_id = '{$result_sp['id']}' AND i.invoice_date LIKE '$rep_year%' AND i.cancel = 'no'");
		...
		<?php include('topbar.php'); ?>
			...
			<main class="content">
				<div class="container-fluid p-0">
					...
					<div class="row">
					<h1 class="h3 mb-3 col-10">Sales - Product wise</h1>
					</div>
									...
									<form method="post" name="report" action="" >
										...
										<div class="row form-group mb-2">
											...
											<div class="col-4" align="right">
												<input type="button" class="btn btn-primary" value="Print" onclick="get_report_print('Sales Report - Product wise')" >
										...
										<div id="rep_div">
											<table width="100%" class="table table-bordered" id="rep_tbl" >
												...
												$q=1;
												$grand_total = 0;
												$select_sp_t = mysqli_query($con, "SELECT id, name FROM products");
												while($result_sp_t = mysqli_fetch_array($select_sp_t)){
												?>
												<tr>
													<td><?php echo $result_sp_t['name']; ?></td>
													<?php
													$row_tot = 0;
													for($k=1; $k <= 12; $k++){
														...
														$select_sale_t = mysqli_query($con, "SELECT SUM(i.total) FROM invoice i, invoice_details d WHERE i.id = d.invoice_id AND d.prod_id = '{$result_sp_t['id']}' AND i.invoice_date LIKE '$newdate%' AND i.cancel = 'no'");
